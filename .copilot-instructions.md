...
### Key Goals
- ⚡ **Maximum Performance**: ZSH startup ~110ms (92% faster than original)
- 🎨 **Beautiful UI**: Git/Venv/K8s aware prompts with consistent colors
- 🛡️ **Reliability**: Graceful fallbacks for missing tools, production-ready
- 📚 **Documentation**: Easy setup guides and system compatibility checks
...
- ✅ **Tested on**: Ubuntu/Debian with zsh 5.9, git 2.43, tmux 3.4
- ✅ **Required**: zsh, git, tmux
- ✅ **Optional**: nvim, kubectl, helm, docker, npm, asdf, bat
- ✅ **Fallbacks**: vim/nano, xsel/xclip/wl-copy/pbcopy
...
```
~/.dotfiles/
├── zshrc                      # Main ZSH config (optimized, 200+ lines)
├── tmux.conf                  # Tmux config (optimized 3.4+, 250+ lines)
├── kitty.conf                 # Kitty terminal config
├── .dircolors                 # Color palette for ls (20+ file types)
├── check-system.sh            # System compatibility checker
├── INSTALLATION.md            # Complete setup guide
├── zsh.d/
│   ├── config.zsh             # Variables & settings
│   ├── tools.zsh              # Editor, paths, npm lazy-load
│   ├── alias.zsh              # Docker aliases (functions, not if-checks)
│   ├── autocomplete.zsh       # Kubectl/helm/kind lazy completions
│   └── tmux.zsh               # Tmux helpers (deprecated, in main config)
└── hack/                      # Scripts & tools
```
...
### 1. ZSH Optimization
- ✅ **Lazy-loading**: Plugins (Zinit), completions, npm
- ✅ **Async plugins**: turbo mode with wait delays (wait"1", wait"2", wait"3")
- ✅ **Compinit caching**: Uses -C flag for speed
- ✅ **Reduced history**: 5000 commands (vs 10000)
- ✅ **Removed heavy plugins**: history-search-multi-word
- ✅ **Smart error handling**: Checks for git/Zinit before using them
...
### 3. Tmux Integration
- ✅ **Mouse**: Fully enabled and kitty-compatible
- ✅ **Copy-mode**: Vi keybindings with optimized scrolling
- ✅ **Aggressive-resize**: For multi-client setups
- ✅ **Extended keys**: Ctrl+Up, M-PageUp support
- ✅ **Focus events**: Better tool integration

### 4. Color Support
- ✅ **Dircolors**: Custom palette in ~/.dircolors
- ✅ **Aliases**: ls/grep/diff with --color=auto
- ✅ **File types**: Archives (red), images (magenta), code (yellow), etc.
- ✅ **Man pages**: LESS_TERMCAP variables configured

### 5. Compatibility Features
- ✅ **Tool detection**: All tools checked with `command -v`
- ✅ **Path safety**: Only add to PATH if directory exists
- ✅ **Editor fallback**: nvim → vim → nano
- ✅ **Conditional loading**: Plugins only load if Zinit available
...
5. **Keep** history size ≤ 5000 for speed

### Compatibility
1. **Check** for tool existence before using (e.g., `command -v nvim`)
2. **Provide** fallbacks (nvim → vim → nano)
...
**Maintainer**: cosckoya
**Status**: ✅ Fully tested on Ubuntu/Debian
