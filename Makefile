...
terraform:
	...
	unzip ${TERRAFORM_FILE} -d ${HOME}/bin ;\
	rm ${TERRAFORM_FILE} ;\
	terraform version
	@echo "Terraform done!"
...
kubectl:
	...
	wget -q ${KUBECTL_URL} -O ${HOME}/bin/kubectl ;\
...
helm:
	...
	wget -q ${HELM_URL} ;\
	tar -xf ${HELM_FILE} --strip-components=1 -C ${HOME}/bin ;\
	rm ${HELM_FILE} ${HOME}/bin/LICENSE ${HOME}/bin/README.md ;\
	helm version --short
	@echo "Helm done!"
...
google:
	...
	sudo bash -c "apt update -qq && apt install -qq -y --no-install-recommends google-cloud-sdk"
...
cheat:
	...
	ln -s ${DOTFILES}/cheats ${HOME}/.config/cheat/cheatsheets/personal
	@echo "Cheat done!"
...
clean:
	@echo "Deleting all generated profiles" ;\
